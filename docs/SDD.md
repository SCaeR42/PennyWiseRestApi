...
| POST | `/api/v1/auth/refresh` | Обновление токена (ротация) |
| POST | `/api/v1/auth/logout` | Отзыв всех refresh-токенов пользователя |

**Модель ревокации — раздельно для access- и refresh-токенов**, а не единый blacklist на всё:

- **Access-токен** — полностью stateless, без проверки в БД на каждый запрос (см. 2.3 — иначе теряется весь смысл JWT при масштабировании `app` на несколько реплик). Цена этого: после logout/кражи он остаётся технически валиден максимум до истечения `JWT_TTL` (по умолчанию 900 сек — см. 6.2). Это осознанный компромисс, а не недосмотр.
- **Refresh-токен — stateful и одноразовый (rotation).** Таблица `refresh_tokens` (`user_id`, `jti`, `expires_at`, `revoked_at`) — не часть финансового домена, поэтому не показана на ER-диаграмме 4.1, как и другая auth-инфраструктура. При каждом `/auth/refresh`: предъявленный `jti` ищется в таблице; если найден и не отозван — тут же помечается отозванным и выдаётся новая пара (старый refresh-токен больше не примется). Если предъявлен уже отозванный (то есть ранее использованный) `jti` — это признак кражи токена: немедленно отзываются **все** активные refresh-токены пользователя, запрос отклоняется с `401 REFRESH_TOKEN_REUSED`.
- **Logout** отзывает все активные refresh-токены пользователя — это разлогинивание "везде" (сессии по устройствам не разделяются, см. 10 — future improvements).

Так как refresh предъявляется на порядок реже, чем обычные запросы (примерно раз в `JWT_TTL`, а не на каждый вызов API), поход в БД здесь не создаёт той нагрузки, которая была бы при проверке блэклиста access-токенов на каждый запрос.

**Таблица `refresh_tokens`** (миграция `009_create_refresh_tokens_table.sql`):

| Поле | Тип | Описание |
|------|-----|----------|
| id | BIGINT UNSIGNED PK | Идентификатор |
| user_id | BIGINT UNSIGNED FK | Владелец |
| jti | CHAR(16) UNIQUE | `jti` из payload refresh-JWT |
| expires_at | TIMESTAMP | Совпадает с `exp` токена — защита на случай рассинхронизации с JWT-проверкой |
| revoked_at | TIMESTAMP NULL | NULL — токен активен; иначе — момент ротации/отзыва |
...
`is_default` эксклюзивен на пользователя: при создании/обновлении кошелька с `is_default=true` сервисный слой в той же БД-транзакции сначала снимает флаг со всех остальных кошельков этого пользователя (`WalletRepository::clearDefaultForUser()`), а затем проставляет его новому — на уровне БД второй `is_default=true` невозможен.
...
Вложенность категорий ограничена **одним уровнем** (root-категории и их прямые дети, без внуков): `parent_id` обязан указывать на категорию без собственного родителя, и категория с уже существующими детьми не может сама получить родителя. Помимо продуктового упрощения, это структурно исключает циклы `parent_id` любой длины (A→B→C→A и т.п.) — не только прямое "сам себе родитель", которое проверялось и раньше.
...
- [ ] Мгновенный отзыв access-токенов через Redis-блэклист (`jti` + TTL = остаток жизни токена, проверяется в `AuthMiddleware`) — закрывает окно между `/auth/logout` и естественным истечением access-токена (сейчас — до `JWT_TTL`, см. 3.2.2). Цена: новый сервис в инфраструктуре и поход в кэш на каждый авторизованный запрос вместо чистого stateless-JWT
- [ ] Управление сессиями по устройствам: `GET /auth/sessions` (список активных refresh-токенов/логинов), `DELETE /auth/sessions/{id}` — точечный logout конкретного устройства вместо текущего "выйти везде"
